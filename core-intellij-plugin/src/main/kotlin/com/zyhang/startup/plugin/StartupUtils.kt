...
internal class StartupUtils {
    companion object {
        ...
        fun PsiAnnotation.parseId(): String {
            return this.findAttributeValue("id")!!.value()
        }
        ...
        private fun PsiElement.value(): String {
            return when (this) {
                ...
                is KtStringTemplateExpression -> this.text
                isKtExpression() -> {
                    ((this as KtElement).resolveToCall()!!.resultingDescriptor as VariableDescriptor)
                        .compileTimeInitializer!!.toString() // use toString() not value(), because other expression use text
                }
